...
#Control data for emergent properties (Fig 1)
Fig1_EmergPropControls = read.csv("~/InputData/Fig1_EmergPropControls.csv")
Fig1_EmergPropControls$Exp = gsub("Cell Growth", "CellNoNorm",  Fig1_EmergPropControls$Exp)

#Control data for Morphology (Fig 1)
Fig1_MorphControls = read.csv("~/InputData/Fig1_MorphControls.csv")

#Control data for qPCR (Fig 1)
Fig1_qPCR = read.csv("~/InputData/Fig1_qPCR.csv")
Fig1_qPCR = na.omit(Fig1_qPCR)

#Data for temporal signalling (Fig 2)
Fig2_StdSignalling = read.csv("~/InputData/Fig2_StdSignalling.csv")
Fig2_StdSignalling$Protein = gsub("pFgfr", "p-Fgfr", Fig2_StdSignalling$Protein)

#Data for signalling pathway cross talk (Fig 2)
Fig2_CrossTalk = read.csv("~/InputData/Fig2_CrossTalk.csv")

#Data for models (Figs 3, 4, S2-10)
Fig3_4_ComboInhibData = read.csv("~/InputData/Fig3_4_ComboInhibData.csv")
Fig3_4_ComboInhibData$Combo = paste(ifelse(Fig3_4_ComboInhibData$U == 1,  "U", ""), ifelse(Fig3_4_ComboInhibData$S == 1,  "S", ""),ifelse(Fig3_4_ComboInhibData$L == 1,  "L", ""),ifelse(Fig3_4_ComboInhibData$R == 1,  "R", ""),ifelse(Fig3_4_ComboInhibData$P == 1,  "P", ""), sep="")
Fig3_4_ComboInhibData$Combo = sapply(strsplit(Fig3_4_ComboInhibData$Combo, ""), paste, collapse="+")
Fig3_4_ComboInhibData$Combo <- sub("^$", "DMSO", Fig3_4_ComboInhibData$Combo)
colnames(Fig3_4_ComboInhibData)[1] = "Name"
colnames(Fig3_4_ComboInhibData) = gsub("\\.", "", colnames(Fig3_4_ComboInhibData))

#BRANN model outputs computed by Dave Winkler  (Figs 3, 4, S8)
Fig3_4_BRANNResults = read.csv("~/InputData/Fig3_4_BRANNResults.csv")

#Data for functional assay (Fig S6, S11)
FigS11_FunctionalAssay = read.csv("~/InputData/FigS11_FunctionalAssay.csv")
FigS11_FunctionalAssay$X = NULL
FigS11_FunctionalAssay = na.omit(FigS11_FunctionalAssay)
...
```{r}
########## 1B: MORPHOLOGY ##########
Fig1_MorphControls$Condition = factor(Fig1_MorphControls$Condition, levels = c("1000 U/mL LIF", "330 U/mL LIF", "330 U/mL LIF + Pro", "0 U/mL LIF"))
ANOVAResTable = as.data.frame(DunnettTest(x=Fig1_MorphControls$Value, g=Fig1_MorphControls$Condition)[[1]])
row.names(ANOVAResTable) = gsub("-1000 U/mL LIF", "", row.names(ANOVAResTable))
ANOVAResTable$Sig <- ifelse(ANOVAResTable$pval <0.05,  "*", NA)


GoIControls_Summarised = Fig1_MorphControls %>% group_by(Condition) %>% summarise(n = n(), Mean = mean(Value), stdev = sd(Value), se=sd(Value)/sqrt(n())) 
...
Fig1_MorphControls_merge = merge(MorphwStats, Fig1_MorphControls, by = "Condition")
...
GoIControls = subset(Fig1_EmergPropControls, Fig1_EmergPropControls$Exp %in% GoI)
...
for(Dayz in c(2,4,6)){
Fig1_EmergPropControls_Subs = subset(GoIControls, GoIControls$Day %in% Dayz)
Control_GraphInputs = Fig1_EmergPropControls_Subs %>% group_by(Condition, Day) %>% summarise(n = n(), Mean = mean(Value), stdev = sd(Value), se=sd(Value)/sqrt(n()))

Fig1_EmergPropControls_Subs$Condition = factor(Fig1_EmergPropControls_Subs$Condition, levels = c("1000 U/mL LIF", "330 U/mL LIF", "330 U/mL LIF + Pro", "0 U/mL LIF"))
ANOVAResTable = as.data.frame(DunnettTest(x=Fig1_EmergPropControls_Subs$Value, g=Fig1_EmergPropControls_Subs$Condition)[[1]])
row.names(ANOVAResTable) = gsub("-1000 U/mL LIF", "", row.names(ANOVAResTable))
ANOVAResTable$Sig <- ifelse(ANOVAResTable$pval <0.05,  "*", NA)
...
Fig1_EmergPropControls_Subs_merge = merge(SummedwStats, Fig1_EmergPropControls_Subs, by = "Condition")
...
Fig1_EmergPropControls_Subs_merge$Day = paste("Day", Fig1_EmergPropControls_Subs_merge$Day.x)
CompileData2 = rbind(CompileData2, Fig1_EmergPropControls_Subs_merge)
...
GoIControls = subset(Fig1_qPCR, Fig1_qPCR$Gene %in% GoI)
...
Fig1_EmergPropControls_Subs = subset(GoIControls, GoIControls$Condition %in% Condit)
ControlPCR_GraphInputs = Fig1_EmergPropControls_Subs %>% group_by(Condition, Day, Gene) %>% summarise(n = n(), Mean = mean(Value), stdev = sd(Value), se=sd(Value)/sqrt(n())) 

ControlGroup = as.data.frame(matrix(ncol = 0, nrow = max(as.data.frame(table(Fig1_EmergPropControls_Subs$Day))$Freq)))  
ControlGroup$Condition = Condit
ControlGroup$Gene = GoI
ControlGroup$Day = "Day 0"
ControlGroup$Value = 0

DataForANOVA = rbind(Fig1_EmergPropControls_Subs, ControlGroup)
...
Fig1_EmergPropControls_Subs$Day = gsub("Day ", "D", Fig1_EmergPropControls_Subs$Day)
Fig1_EmergPropControls_Subs$Day = factor(Fig1_EmergPropControls_Subs$Day, levels = c("D2", "D4", "D6"))
...
SubB = subset(Fig1_qPCR, Fig1_qPCR$Condition %in% x & Fig1_qPCR$Gene %in% Data_To_Plot)

for(y in c("Rex1", "Oct4", "Dnmt3b", "Fgf5",  "Lefty2", "Otx2", "Mixl1")){
SubC = subset(SubA, SubA$Gene %in% y)
SubD = subset(Fig1_qPCR, Fig1_qPCR$Gene %in% y & Fig1_qPCR$Condition %in% x)
...
Fig2_StdSignalling2 = Fig2_StdSignalling

Fig2_StdSignalling2$Value = log2(Fig2_StdSignalling2$Value) 
Fig2_StdSignalling2$Time = factor(Fig2_StdSignalling2$Time, levels = c("0 h", "0.16 h", "0.5 h", "1 h", "2 h", "6 h", "12 h", "24 h", "48 h", "96 h", "144 h" ))
Fig2_StdSignalling2$Protein = factor(Fig2_StdSignalling2$Protein, levels = c("p-Stat3@Y705", "p-Stat3@S727", "p-Fgfr", "p-Erk1/2", "p-Akt", "p-S6k", "p-Rps6",     "p-4epb1"))


Constituative = list()
for(Proteinz in rev(c("p-4epb1", "p-Rps6", "p-S6k", "p-Akt", "p-Erk1/2", "p-Fgfr", "p-Stat3@S727", "p-Stat3@Y705"))){
SubsDataForANOVA = subset(Fig2_StdSignalling2, Fig2_StdSignalling2$Protein %in% Proteinz)
...
Fig2_CrossTalk = read.csv("~/Dropbox/PhD/MAR20 WIP - Stat Modelling/2021 Analysis/CrossTalkData_NormToES_Long.csv")
Inhib2H = list()
for(p in c("pERK", "pRPS6")){
if(p == "pERK"){p2 = "p-Erk1/2"}else{p2 = "p-Rps6"}
Reduced_phospho_2h0h = subset(Fig2_CrossTalk, Fig2_CrossTalk$Protein %in% p & Fig2_CrossTalk$Time %in% c("2", "0"))
...
Fig2_CrossTalk$Combo = paste(Fig2_CrossTalk$Inhib1, Fig2_CrossTalk$Inhib_Dash, sep=" ")
Fig2_CrossTalk$Combo = gsub("NA", "-", Fig2_CrossTalk$Combo)
Fig2_CrossTalk$Combo = factor(Fig2_CrossTalk$Combo, levels = c("- -", "U -", "U U", "S -", "S S", "L -", "L L", "R -", "R R", "P -", "P P", "U S", "U L", "U R", "U P", "R U", "R S", "R L", "R P"))
Fig2_CrossTalk$Value = log2(Fig2_CrossTalk$Value+0.01)
SubsDataForTable = Fig2_CrossTalk %>% group_by(Protein, Time, Combo) %>% dplyr::summarise(Mean = mean(Value), stdev = sd(Value), se=sd(Value)/sqrt(n())) 
...
CrossTalkInput = subset(Fig2_CrossTalk, Fig2_CrossTalk$Protein == Proteinz)
...
levels(Fig2_CrossTalk$Combo) <- gsub(" ", "\n", levels(Fig2_CrossTalk$Combo))
Fig2_CrossTalk_ForPlot = subset(Fig2_CrossTalk, ! Fig2_CrossTalk$Time %in% c(0,2, "0", "2"))
...
########## DATA PREP ##########
SingleMetric_AllData =  Fig3_4_ComboInhibData[, c("Name", "Combo", "U", "S", "L", "R", "P", GoI)]
...
########## PANEL II - AVP ##########    
PullBRANNData = subset(Fig3_4_BRANNResults, Fig3_4_BRANNResults$Exp %in% GoI_X)
...
scientific_10 <- function(x) {
  ...
  parse(text= gsub("e\\+", "%*%10^", gsub("e\\+0", "%*%10^", gsub(" ", "", gsub("−", "-", scales::scientific_format(digits=2)(x))))))
} 
...
for(x in c(2,4,6)){
SingleMetric_SingleDay = Fig3_4_ComboInhibData[, c("Name", "Combo", "U", "S", "L", "R", "P", paste(GoI, x, sep="_"))]
...
pdf(paste(FilePath, "FigureS8_Prolif.pdf", sep=""), width = 15, height = NRows*4)
...
pdf(paste(FilePath, "Figure3BCDE_EmergPropModels.pdf", sep=""), width = 12, height = 4*2.5)
print(Figure3)
dev.off()
```
...
########## PANEL II - AVP ##########    
PullBRANNData = subset(Fig3_4_BRANNResults, Fig3_4_BRANNResults$Exp %in% GoI)
PullAvP = AllAVP_GoI[, c("Response", "Predicted", "Exp", "ModelType")]
FourModels = rbind(PullBRANNData, PullAvP)
...
pdf(paste(FilePath, "Figure4_GEModels.pdf", sep= ""), width = 12, height = 4*2.5)
print(Figure4Main)
dev.off()


pdf(paste(FilePath, "FigureS10_GeneExp_TwoWay.pdf", sep= ""), width = 12, height = 4*2.5)
print(Figure4Supp)
dev.off()
```
...
```{r}
SumForCorPlot = Fig3_4_ComboInhibData %>% dplyr::select(FunctionalAssay, CellNoNorm_2, CellNoNorm_4, CellNoNorm_6, Prolif_2, Prolif_4, Prolif_6, Apoptosis_2,  Apoptosis_4, Apoptosis_6, Morphology, Rex1, Oct4, Dnmt3b, Fgf5, Mixl1, Lefty2, Otx2)
...
###
SumForCorPlot = Fig3_4_ComboInhibData %>% select(FunctionalAssay, Rex1, Oct4, Dnmt3b, Fgf5, Mixl1, Lefty2, Otx2)
colnames(SumForCorPlot) = gsub("_", " - Day ", gsub("FigS11_FunctionalAssay", "Functional assay", gsub("Prolif", "Proliferation", colnames(SumForCorPlot))))
...
##FIGURE S7: PCA
```{r}
Fig3_4_ComboInhibData2 = Fig3_4_ComboInhibData

Fig3_4_ComboInhibData2$U = NULL; Fig3_4_ComboInhibData2$S = NULL; Fig3_4_ComboInhibData2$L = NULL; Fig3_4_ComboInhibData2$R = NULL; Fig3_4_ComboInhibData2$P = NULL; Fig3_4_ComboInhibData2$FigS11_FunctionalAssay = NULL; Fig3_4_ComboInhibData2$Name = NULL;  Fig3_4_ComboInhibData2$CellNoRaw_2 = NULL;  Fig3_4_ComboInhibData2$CellNoRaw_4 = NULL;  Fig3_4_ComboInhibData2$CellNoRaw_6 = NULL
Fig3_4_ComboInhibData3 = Fig3_4_ComboInhibData2 %>% group_by(Combo) %>% summarise_all(mean, na.rm=T)
Fig3_4_ComboInhibData3 = na.omit(Fig3_4_ComboInhibData3)
RowNames = Fig3_4_ComboInhibData3$Combo

Fig3_4_ComboInhibData3$Combo = NULL

Fig3_4_ComboInhibData_Zscore = as.data.frame(scale(Fig3_4_ComboInhibData3))
row.names(Fig3_4_ComboInhibData_Zscore) = RowNames

#########
set.seed(123)
OutputPCA = list()
OutputPCA[["silhouette"]] = fviz_nbclust(Fig3_4_ComboInhibData_Zscore, kmeans, method = "silhouette", linecolor = ColourFiveWay, k.max = 6) +labs(title = element_blank()) + theme(axis.title = element_text(face = "bold", size = 8), axis.text = element_text(face = "bold", size = 8))
k2 <- kmeans(Fig3_4_ComboInhibData_Zscore, centers = 2, nstart = 25)

OutputPCA[["Main"]] = fviz_cluster(k2, data = Fig3_4_ComboInhibData_Zscore, labelsize = 6, show.clust.cent	= F) + scale_color_manual(values = c("1" = ColourLinear, "2" = ColourTwoWay)) + scale_fill_manual(values = c("1" = ColourLinear, "2" = ColourTwoWay)) +theme(legend.position = "none", plot.title = element_blank(), axis.text = element_text(face = "bold", size = 8), axis.title = element_text(face = "bold", size = 8)) #, ggtheme = theme_bw()

kmean_cluster = as.data.frame(k2$cluster)
Fig3_4_ComboInhibData3 = as.data.frame(Fig3_4_ComboInhibData3)
row.names(Fig3_4_ComboInhibData3) = RowNames
Fig3_4_ComboInhibData4 = merge(Fig3_4_ComboInhibData3, kmean_cluster, by = 0)
...
PullData = Fig3_4_ComboInhibData4[,c(dt,"k2$cluster")]
...
pdf(paste(FilePath, "FigureS7_PCA.pdf", sep=""), width = 10, height = 12)
print(OUTPUT2)
dev.off()
...
MaxVal = max(FigS11_FunctionalAssay$Value) *1.1
MinVal = min(FigS11_FunctionalAssay$Value)*1.1
FigS11_FunctionalAssay$Exp = NULL
for(x in Condit){
FA_ForSTATS = subset(FigS11_FunctionalAssay, FigS11_FunctionalAssay$Condition == x)
...
for(x in unique(Fig3_4_BRANNResults$Exp)){
Subs = subset(Fig3_4_BRANNResults, Fig3_4_BRANNResults$Exp == x)  
