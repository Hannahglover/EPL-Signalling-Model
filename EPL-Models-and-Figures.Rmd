...
###FIGURE 3, PANEL B: Correlation Plot
```{r}
...
#
Data_To_Plot = c("Prolif_Ntile_8")# "CellNoNorm", "Apoptosis_Log", "Prolif_Power_6") 
GEXPanel = list()
...
###FIGURE 5: 5 Coef Figures v2 - Has Days 2,4 and 6 expression for controls
```{r}
...
### SUPPLEMETAL DATA: Functional Assay
```{r}
FA_list = list()
...
```{r}
SW_Res = as.data.frame(matrix(ncol=2, nrow=0))
colnames(SW_Res) = c("SWpval", "Exp")

for(x in unique(BRANN_AvP$Exp)){
Subs = subset(BRANN_AvP, BRANN_AvP$Exp == x)  
BRANN_SW = as.data.frame(shapiro.test(Subs$Predicted)[[2]])  
colnames(BRANN_SW) = "SWpval"
BRANN_SW$Exp = x
...
#TO DO
- F1: geom_point on pERK and pRPS6
- S7-10:Raws for supplemental - more data at day 6 than earlier
- Geom_point on all i) controls
- Facet wrap for functional assay to show data points and error?
- N for above small scale experiments
- hline 0



##Export Datasets for new BRANNs
```{r}
############## EXPORT APOPTOSIS ##############
DatasetsList = list()
SingleMetric_AllData = as.data.frame(matrix(ncol=9, nrow=0))
...
Reduced_ThreePlus$Day2 =  ifelse(Reduced_ThreePlus$Day == 2,  1, 0)
Reduced_ThreePlus$Day4 =  ifelse(Reduced_ThreePlus$Day == 4,  1, 0)
Reduced_ThreePlus$Day6 =  ifelse(Reduced_ThreePlus$Day == 6,  1, 0)
Reduced_ThreePlus$LogResponse = log(Reduced_ThreePlus$Response)


Apop_Std = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "Response")]
write.table(Apop_Std, paste("FullDataset_", "ApopBinaryUntrans", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")

Apop_Log = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "LogResponse")]
colnames(Apop_Log) = gsub("Log", "", colnames(Apop_Log))
write.table(Apop_Std, paste("FullDataset_", "ApopBinaryLog", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")
shapiro.test(Reduced_ThreePlus$Response) #Good
...
Reduced_ThreePlus$Day4 =  ifelse(Reduced_ThreePlus$Day == 4,  1, 0)
Reduced_ThreePlus$Day6 =  ifelse(Reduced_ThreePlus$Day == 6,  1, 0)
Reduced_ThreePlus$ResponsePower = Reduced_ThreePlus$Response^6
Reduced_ThreePlus$ResponseNtile = ntile(Reduced_ThreePlus$Response, 8)

Prolif_Std = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "Response")]
write.table(Prolif_Std, paste("FullDataset_", "ProlifBinaryUntrans", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")


Prolif_Power6 = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "ResponsePower")]
colnames(Prolif_Power6) = gsub("Power", "", colnames(Prolif_Power6))
write.table(Apop_Std, paste("FullDataset_", "Prolif_Power6", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")


Prolif_Ntile8 = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "ResponseNtile")]
colnames(Prolif_Ntile8) = gsub("Ntile", "", colnames(Prolif_Ntile8))
write.table(Apop_Std, paste("FullDataset_", "Prolif_Ntile8", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")
shapiro.test(Reduced_ThreePlus$Response) #Good
...
DatasetsList = list()
SingleMetric_AllData = as.data.frame(matrix(ncol=9, nrow=0))
colnames(SingleMetric_AllData) = c("Name", "Combo", "U", "S", "L", "R", "P", "Day", "CellNoNorm")

for(x in c(2,4,6)){
SingleMetric_SingleDay = AllVariables[, c("Name", "Combo", "U", "S", "L", "R", "P", paste("CellNoNorm", x, sep="_"))]
SingleMetric_SingleDay$Day = x
colnames(SingleMetric_SingleDay) = gsub("_.*", "", colnames(SingleMetric_SingleDay))
SingleMetric_AllData = rbind(SingleMetric_AllData, SingleMetric_SingleDay)
}
...
SingleMetric_AllData$Name = gsub(" ", "", SingleMetric_AllData$Name)
SingleMetric_AllData$Response = SingleMetric_AllData[["CellNoNorm"]]
SingleMetric_AllData[["CellNoNorm"]] = NULL
DatasetsList[["Full Dataset - Day Numeric"]] = SingleMetric_AllData
SingleMetric_AllData$Name = NULL
SingleMetric_AllData$Combo = paste("Day", SingleMetric_AllData$Day, "_", SingleMetric_AllData$Combo, sep="")
...
CellNoNorm_Std = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P", "Day2", "Day4", "Day6", "Response")]
write.table(CellNoNorm_Std, paste("FullDataset_", "CellNoNormBinaryUntrans", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")
shapiro.test(Reduced_ThreePlus$Response) #Good
...
Morph_Log = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P",  "LogResponse")]
colnames(Morph_Log) = gsub("Log", "", colnames(Morph_Log))
write.table(Morph_Log, paste("FullDataset_", "Morph_Log", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")
shapiro.test(Reduced_ThreePlus$Response) #Good
...
############## EXPORT Oct4 ##############
SingleMetric_AllData =  AllVariables[, c("Name", "Combo", "U", "S", "L", "R", "P", "Oct4")]
SingleMetric_AllData$Response = SingleMetric_AllData[["Oct4"]]
SingleMetric_AllData[["Oct4"]] = NULL
SingleMetric_AllData = na.omit(SingleMetric_AllData)
DatasetsList[["Full Dataset"]] = SingleMetric_AllData
##DATA PREP - AVERAGED DATASET
SingleMetric_AllData$Name = NULL
...
Oct4_Std = Reduced_ThreePlus[, c("Combo", "U", "S", "L", "R", "P",  "Response")]
write.table(Oct4_Std, paste("FullDataset_", "Oct4_Std", "_", Sys.Date(), ".txt", sep=""), sep="\t",row.names=FALSE, quote=F,  eol = "\n")
shapiro.test(Oct4_Std$Response) #Good
```
...
#####EXPLORATORY  ANALYSIS: FOR 5 COEFS
```{r}
y = "Dnmt3b"
p = "Full Dataset"
z = "Linear Model"
...
###FIGURE 5: 5 Coef Figures
```{r}
Data_To_Plot = c("Lefty2", "Otx2") # #"Rex1", "Oct4", "Dnmt3b", "Fgf5", "Mixl1" #"Lefty2", "Otx2"
GEXPanel = list()
NRows = length(Data_To_Plot)
...
########## PANEL I - CONTROLS ##########
GoIControls = subset(EPLControlData, EPLControlData$Exp %in% GoI)
GoIControls_Summarised = GoIControls %>% group_by(Condition) %>% summarise(Mean = mean(Value), stdev = sd(Value), se=sd(Value)/sqrt(n())) 
GoIControls_Summarised$Condition = factor(GoIControls_Summarised$Condition, levels = c("1000 U/mL LIF", "330 U/mL LIF", "330 U/mL LIF + Pro", "0 U/mL LIF"))
GEXPanel[[paste(GoI, "_i_Control", sep="")]] = ggplot(GoIControls_Summarised, aes(x=Condition, y=Mean)) + geom_col(fill = ColourNeutral, color = "#000000")  +theme_classic() + geom_hline(yintercept=0) + ylab(expression(Log[2]~Fold~Change)) + xlab("") + geom_errorbar(aes(ymin = Mean-se, ymax = Mean+se), position = position_dodge(width = 0.9), width = 0.2) + theme(legend.position="none", plot.title = element_text(size = 15, face = "bold.italic")) + scale_x_discrete(labels = function(x) str_wrap(x, width = 8.5)) + ggtitle(GoI)
...
Figure4 = ggarrange(plotlist = GEXPanel, ncol = 4, nrow=NRows, widths = rep(c(0.85,1.1,0.85,2), NRows))
pdf("ModellingPaper_Figure4.pdf", width = 15, height = NRows*4)
print(Figure4)
dev.off()
